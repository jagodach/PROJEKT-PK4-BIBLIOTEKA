...
void AddUser(bool bPrintMenuInfo, bool bAddAdmin)
{
	...
	LibrarySystem::User* newUser = LibrarySystem::Globals::LibraryManager->AddUser(userStream.str());
...
void AddAuthor(bool bPrintMenuInfo)
{
	...
	LibrarySystem::Author* newAuthor = LibrarySystem::Globals::LibraryManager->AddAuthor(authorStream.str());
...
void AddItem(bool bPrintMenuInfo)
{
	...
	if (authors.size() > 0)
	{
		...
		LibrarySystem::LibraryItem* newItem = LibrarySystem::Globals::LibraryManager->AddItem(itemStream.str(), itemType);
	...
	while (doLoop)
	{
		...
		case 4:
			...
			if (bAdminView)
			{
				...
				std::cin.get();
				std::cin >> index;
				user = LibrarySystem::Globals::LibraryManager->GetUser(index);
			}
