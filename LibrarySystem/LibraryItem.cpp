...
namespace LibrarySystem
{
	...
	void LibraryItem::BindAuthor(std::shared_ptr<Author> author)
	{
		m_author = author;
	}
	...
	std::string LibraryItem::GetDescription()
	{
		...
		stream << m_name  << ". Author: " << m_author->GetFullName() << ". Created at: " << m_creationDate.ToString();
		return stream.str();
	}
	...
	std::string LibraryItem::ToString()
	{
		...
		std::stringstream stream;
		stream << "Item id: " << m_Id << ". " << m_name << " - " << (m_bIsAvailable ? "is available" : "isn't available") << ". Author: " << m_authorId
	...
	{	
		...
		stream << m_Id << ";" << m_lendPrice << ";" << m_bIsAvailable << ";" << Helpers::ConvertEnumToString(m_ItemType) << ";"
			<< m_authorId << ";" << m_creationDate.ToCsvRecord() << ";" + m_addedToLibraryDate.ToCsvRecord() << ";" << m_name;
		return stream.str();
	}
}
